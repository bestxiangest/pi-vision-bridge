...
describe("resilience", () => {
	it("classifies retryable, fatal, and abort errors", () => {
		...
		assert.equal(classifyError({ status: 501 }), "retryable");
		assert.equal(classifyError({ status: 599 }), "retryable");
	...
	it("treats provider timeout errors as retryable unless the request was aborted", () => {
		assert.equal(classifyError(new DOMException("The request timed out", "TimeoutError")), "retryable");
	});
