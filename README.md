...
对目标端点（OpenAI 兼容 Chat Completions）实测，视觉调用耗时几乎完全由**输出 token 数**决定：生成速率约 100–130 token/秒，另加约 2 秒固定开销。旧版提示词要求模型填充完整契约（observations + text_blocks + design_spec + comparison），模型会写出 2500+ token 的冗长观测（20–40 秒甚至更久）。新版算法做了三项实测验证的改动，把典型调用压到 10–15 秒、最坏情况从 2 分钟+ 降到 ~20 秒：

1. **极简提示词**：用户侧提示压缩到 ~500 字符（原来 ~2000 字符），shape 按模式裁剪——`general` 模式只要求 summary + ≤5 条 observations + ≤2 条 uncertainties，不再包含 text_blocks/design_spec。提示词长度会直接拉长推理模型的思考（实测同样请求下 454 字符提示触发 ~1500 字符推理、7 秒返回，而 1300 字符提示触发 8000+ 字符推理、15 秒+）。
2. **低温采样（temperature 0.1）**：这是对推理模型最大的杠杆。默认温度 0.5 下推理普遍 3000–8000+ 字符，0.1 下降到 ~1300–2100 字符，且输出更确定（有效 JSON 比例大幅提升）。
3. **输出上限保底 + 空响应重试**：`max_tokens` 按 `Response detail` 分级（`concise` 2048 / `balanced` 4096 / `detailed` 8192）。注意上限不能设太紧：推理 token 不受控，若预算被思考吃光，模型返回**空内容**，整次调用作废。收到空内容时不再走“修复”路径（对空字符串修复会编造证据且双重延迟），而是标记为可重试错误、以全新请求重试。
...
视觉调用不会接收主会话上下文，其耗时不随会话长度变化（`test/context-isolation.test.ts` 在 provider 层验证：发送给视觉 provider 的 context 永远只有视觉系统提示 + 一条含 objective 与图片的用户消息）。端到端变慢主要来自主模型在长上下文上的工具调用往返。执行 `/vision-audit` 对比新旧会话中视觉调用的 `elapsedMs` 可以确认这一点。若单次视觉调用本身就很慢，优先检查：图片长边是否落在更贵的瓦片档位（见“上传编码与视觉延迟”的 1792/1920/2048 分档）、`Response detail` 是否为 `detailed`（输出上限 8192）、以及端点的推理模型当前是否处于深度思考状态（此时可依赖空响应重试自动换一轮）。
