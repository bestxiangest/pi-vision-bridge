...
| 工具 | 用途 | 典型调用时机 |
| --- | --- | --- |
| `vision_inspect` | 对一张图片执行首次任务化分析（`mode` 省略时默认 `auto`） | 解释/描述图片、UI 逆向、测量、图表或报错截图分析 |
...
| 模式 | 重点 |
| --- | --- |
| `auto`（默认） | **全自动**：一次调用覆盖场景/UI、相关可见文字（在 facts 中逐字引用）与布局——解释“这是什么”时不需要再补一发 OCR |
| `general` | 返回完成当前目标所需的最小视觉证据集 |
| `ocr` | 按阅读顺序提取可见文字并标记不确定字符（整屏转写，输出量大、耗时约 2–3 倍，仅在确实需要全文时使用） |
...
> 一张图片只调用一次：默认 `auto` 已覆盖“这是什么+上面有什么文字+布局如何”。只有任务明确要求全文转写时才用 `ocr`。工具指引中明确禁止对同一张图连跑两种模式。
...
| `Cache` | 是否复用已缓存的视觉结果 |
| `Hedged requests` | 并行发两份相同请求、先到先赢（默认开）；约 2 倍 token 消耗换取稳定的低延迟 |
...
3. **并行双发（Hedged requests，默认开）**：推理长度随机（1400–9000+ 字符），单次请求会随机落入 7 秒快窗或 15 秒+ 深思考窗；同时发两份相同请求、先到先赢、输家立即 abort，把快窗变成常态（min-of-two）。精度与单发完全一致（同 prompt 同模型），代价是首次读取消耗约 2 倍 token——同图同 objective 的结果缓存（见下）会让重读完全免费。可在 `/vision-settings` 关闭。
